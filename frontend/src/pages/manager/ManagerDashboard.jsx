...
function ManagerDashboard() {
  ...
  const fetchPendingRequests = async () => {
    try {
      const response = await fetch("/api/leave-approvals/pending", {
      ...
      setTeamOnLeaveToday(requests.filter(l => l.startDate <= today && l.endDate >= today && l.status === "APPROVED").length);

      const now = new Date();
      setMonthlyRequests(requests.filter(l => {
      ...
      <MainLayout>
        <div className="manager-dashboard">
          ...
          <div className="stats-grid">
            <div className="stat-card"><h3>Pending Requests</h3><p>{pendingRequests.length}</p></div>
            <div className="stat-card"><h3>Team On Leave Today</h3><p>{teamOnLeaveToday}</p></div>
            <div className="stat-card"><h3>This Month Requests</h3><p>{monthlyRequests}</p></div>
          </div>
          ...
          {loading ? <p>Loading requests...</p> : (
              <div className="table-wrapper">
                <table className="requests-table">
                  <thead>
                  <tr>
                    <th>Employee</th><th>Leave Type</th><th>Dates</th><th>Status</th><th>Action</th>
                  </tr>
                  </thead>
                  <tbody>
                  {pendingRequests.length === 0 ? (
                      ...
                      pendingRequests.slice(0, 5).map(leave => (
                          <tr key={leave.id}>
                            ...
                            <td><button onClick={() => navigate("/manager/leave-approvals")}>Review</button></td>
                          </tr>
                      ))
                  )}
                  </tbody>
