...
function LeaveApprovalPage() {
  ...
  const approveLeave = async (id) => {
    try {
      await fetch(`/api/leave-approvals/${id}/approve`, {
        method: "PATCH",
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchRequests();
  ...
  const handleReject = async () => {
    try {
      await fetch(`/api/leave-approvals/${selectedLeaveId}/reject`, {
        method: "PATCH",
  ...
  return (
      <MainLayout>
        <div className="leave-approval-container">
          ...
          <div className="table-wrapper">
            <table className="leave-table">
              <thead>
              <tr>
                <th>Employee</th><th>Leave Type</th><th>Dates</th><th>Days</th><th>Actions</th>
              </tr>
              </thead>
              <tbody>
              {requests.map(leave => (
        ...
        <RejectModal
            show={showModal} remarks={remarks} setRemarks={setRemarks}
            onClose={() => setShowModal(false)} onConfirm={handleReject}
        />
      </MainLayout>
  );
}
